...
export function XwayFunnelDashboard({
  filteredTests,
  filters,
  onStageFilter,
  xwayStatusByTestId,
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
        <div>
          ...
          <p className="text-[12px] text-slate-500 dark:text-slate-400 max-w-2xl" style={{ fontWeight: 500, lineHeight: 1.4 }}>
            Расчёт по XWAY-данным. Клик по этапу отфильтрует тесты точно так же, как на основной вкладке.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="inline-flex items-center h-7 rounded-full border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 text-[12px] text-slate-500 dark:text-slate-400" style={{ fontWeight: 600 }}>
            {xwayStatusText}
          </span>
...
function DonutStage({
  ...
  const gradientId = `xway-donut-${label.replace(/\s/g, "")}-${percent}`;
...
function FunnelCardChart({
  card,
  filters,
  onStageFilter,
  mode,
}: {
  card: ReturnType<typeof abBuildCabinetFunnelCards>[number];
  filters: Filters;
  onStageFilter: (cabinet: string, stage: string, source: string) => void;
  mode: ChartMode;
}) {
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div>
          ...
          <div className="text-[12px] text-amber-500/90 dark:text-amber-300/80 mt-0.5" style={{ fontWeight: 600 }}>
            Без цены: {abFormatInt(card.priceMissingCount)}
          </div>
...
function PendingFunnelCard({ cabinet, mode }: { cabinet: string; mode: ChartMode }) {
  return (
    <div className="border border-slate-200/80 dark:border-slate-700/80 rounded-xl bg-gradient-to-br from-white via-white to-slate-50/50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800/50 p-4 shadow-sm">
      ...
      {mode === "bars" ? (
        <div className="space-y-3">
          {["CTR", "Цена", "CTR x CR1", "Итог"].map((label, index) => (
            <div key={label}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[13px] text-slate-400 dark:text-slate-500" style={{ fontWeight: 600 }}>
                  {label}
                </span>
              ...
              <div className="relative w-full h-[7px] rounded-full bg-slate-100 dark:bg-slate-700/60 overflow-hidden">
                <div
                  ...
                  style={{ width: `${(4 - index) * 18 + 10}%`, background: "linear-gradient(90deg, #e2e8f0, #cbd5e1)" }}
                />
              </div>
            </div>
          ))}
